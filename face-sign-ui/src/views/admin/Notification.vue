<template>
  <div class="notification-container">
    ...
    <el-dialog
        ...
        :close-on-click-modal="false"
    >
      <el-form :model="addNotificationForm" ref="addNotificationForm" label-width="80px">
        <el-form-item label="发送者">
          <el-select v-model="addNotificationForm.senderId" placeholder="请选择发送者">
            <el-option v-for="item in userOptions" :key="getName(item.id)" :label="getName(item.id)" :value="item.id"/>
          </el-select>
        </el-form-item>
...
import { getAllPageNotifications, updateNotification, deleteNotification, saveNotification } from '@/api/notification.js'
...
async function fetchNotifications() {
  loading.value = true
  try {
    const res = await getAllPageNotifications(pageParams, searchForm)
